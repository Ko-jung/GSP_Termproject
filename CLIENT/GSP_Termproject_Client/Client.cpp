...
{
    RECT WindowSize = { 0,0,WINWIDTH,WINHEIGHT };
    PAINTSTRUCT ps;
    ...
    case WM_CREATE:
    {
        ...
        // 시작할 때 자신의 아이디를 입력 받는다.
        std::string ID;
        std::cout << "Input ID :";
        //std::cin >> ID;
        ID = "2019180016";
        Game->SetOwnActorID(ID.c_str());

        Game->InitSocket();
        Game->Connect("127.0.0.1");
        //{
        //    PJoinNewPlayer PJNP(-1);
        //    Game->Send(&PJNP, sizeof(PJNP));
        //    Game->Recv();
        //}
...
{
    Game->recv_callback(errors, r_size, p_wsaover, recv_flag);
}

void CALLBACK send_callback(DWORD errors, DWORD transfer_size, LPWSAOVERLAPPED p_wsaover, DWORD recv_flag)
{
    Game->send_callback(errors, transfer_size, p_wsaover, recv_flag);
}